...
class SimpleDocumentExtractor:
    ...
    def __init__(self, max_workers: int = 4):
        ...
        # Create instance-level supported formats to avoid mutating the class variable
        self.supported_formats = dict(self.SUPPORTED_FORMATS)
    ...
    def _extract_single_file(self, file_path: Path) -> Dict[str, str]:
        ...
        if ext not in self.supported_formats:
            return {str(file_path): f"Error: Unsupported format: {ext}"}
...
Examples:
  ...
  python document_extractor.py /path/to/documents/
  
  # Save output
  python document_extractor.py document.pdf -o output.txt
  
  # JSON output
  python document_extractor.py /path/to/docs/ --json
  
  # Parallel processing
  python document_extractor.py /path/to/docs/ -w 8
        """
    )
